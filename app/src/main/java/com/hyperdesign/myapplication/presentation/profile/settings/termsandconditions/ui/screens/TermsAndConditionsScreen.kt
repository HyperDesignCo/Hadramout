package com.hyperdesign.myapplication.presentation.profile.settings.termsandconditions.ui.screens
...
import com.hyperdesign.myapplication.presentation.profile.settings.returnpolicy.ui.screens.ReturnPolicyScreenContent
import org.koin.androidx.compose.koinViewModel

@Composable
fun TermsAndConditionsScreen(showStaticPagesViewModel: StaticPagesViewModel= koinViewModel()){
    ...
    LaunchedEffect(Unit) {
        showStaticPagesViewModel.handleIntents(StaticPagesIntents.DisplayPage(2))
    }
    LaunchedEffect(pageState) {
        page = pageState.pagesResponse?.pages
    }
    val navController= LocalNavController.current
    page?.let { TermsAndConditionsScreenContent(page = it,onBackPressed = {navController.popBackStack()}) }
}


@Composable
fun TermsAndConditionsScreenContent(page: PageEntity, onBackPressed:()->Unit){
