package com.hyperdesign.myapplication.presentation.profile.settings.privacypolicy.ui.screens
...
import com.hyperdesign.myapplication.presentation.profile.settings.termsandconditions.ui.screens.TermsAndConditionsScreenContent
import org.koin.androidx.compose.koinViewModel


@Composable
fun PrivacyPolicyScreen(showStaticPagesViewModel: StaticPagesViewModel= koinViewModel()){
    ...
    LaunchedEffect(Unit) {
        showStaticPagesViewModel.handleIntents(StaticPagesIntents.DisplayPage(1))
    }
    LaunchedEffect(pageState) {
        page = pageState.pagesResponse?.pages
    }
    val navController= LocalNavController.current
    page?.let { PrivacyPolicyScreenContent(page = it,onBackPressed = {navController.popBackStack()}) }
}


@Composable
fun PrivacyPolicyScreenContent(page: PageEntity, onBackPressed:()->Unit){
    Column(
        modifier = Modifier
            .fillMaxSize()
